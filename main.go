...
func runGrpcServer(config util.Config, store db.Store, taskDistributor worker.TaskDictributor) {
...
func runGatewayServer(config util.Config, store db.Store, taskDistributor worker.TaskDictributor) {
	server, err := gapi.NewServer(config, store, taskDistributor)
	if err != nil {
		log.Fatal().Msg("cannot create server")
	}
